...
import { getStockGroups, createStockGroup, updateStockGroup, deleteStockGroup } from '../../services/stock';
import type { StockGroup } from '../../services/stock';
...
export default function StockGroupList() {
  ...
  /** 打开新增/编辑弹窗 */
  const openModal = (record?: StockGroup) => {
    if (record) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      setLoading(true);
      if (editingId) {
        await updateStockGroup(editingId, values);
        message.success('更新成功');
      } else {
        await createStockGroup(values);
  ...
  const columns: ProColumns<StockGroup>[] = [
